...
module.exports = class extends Monitor {
  ...
  parseCommand (msg) {
    ...
    return {
      command: msg.content.replace(prefix, '').trim().split(' ')[0].toLowerCase(),
      prefix,
      prefixLength
    }
  }
  ...
  getPrefix (msg) {
    ...
    // console.log('regex prefix:', prefix[0])
    if (prefix instanceof RegExp) {
      if (prefix.test(msg.content)) return prefix
    } else if (prefix instanceof Array) {
      for (let i = prefix.length - 1; i >= 0; i--) {
        if (prefix[i] instanceof RegExp) {
          console.log('testing regex prefix:', prefix[i])
          if (prefix[i].test(msg.content)) {
            console.log('msg tested positive:', msg.content, prefix[i])
            return prefix[i]
          }
        } else if (msg.content.startsWith(prefix[i])) return new RegExp(`^${regExpEsc(prefix[i])}`)
      }
